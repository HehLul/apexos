...
function App() {
  useEffect(() => {
    ...
    const sectionsToAnimate = document.querySelectorAll(
      ".why, .features, .how, .pricing, .problem, .solution, .urgent"
    );
    ...
    setTimeout(() => {
      const featureRows = document.querySelectorAll(
        ".feature-row-1, .feature-row-2,  .problem-row-1, .problem-row-2"
      );
      featureRows.forEach((row) => {
        // Create a wrapper div around each row's cards for observation
        const cards = row.querySelectorAll(".feature-card, .problem-card");
        if (cards.length > 0) {
